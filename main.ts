...
let mainDisplay: HTMLElement,
  ...
  seekbarCircle: HTMLElement,
  alermText: HTMLElement,
  ...
  const content = settings
    ...
    .map((v, i) =>
      h(
        ...
        [h('div', { class: 'setup-title' }, v.title), h('div', { class: 'setup-caption' }, SectoString(v.duration))],
        clickItem
      )
    )
...
const playTone = (t0: number, type: OscillatorType = 'square', n: number, t = 0, duration = 0.1) => {
  ctx.resume()
...
const h = (
  tag: string,
  attr: { [key in string]: string } | null = null,
